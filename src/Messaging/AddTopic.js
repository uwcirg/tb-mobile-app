  ...
  return (
    <div className={classes.addTopic}>
      ...
      <div className={classes.main}>
        {!add ? (
          <>
            {t("messaging.creation.fab")}{" "}
            <Fab
              size="small"
              style={{ backgroundColor: Colors.buttonBlue, boxShadow: "none" }}
              onClick={() => {
                messagingStore.newChannel.visible = true;
              }}
            >
          ...
          <>
            <Button
              ...
              onClick={() => {
                messagingStore.newChannel.visible = false;
              }}
            >
              {t("messaging.creation.cancel")}
            </Button>
            <Button
              onClick={messagingStore.submitNewChannel}
              className={classes.create}
            >
              {t("messaging.creation.create")}
            </Button>
          </>
        )}
      </div>
...
const AddDiscussion = observer(() => {
  ...
  const { newChannel, updateNewTitle, updateNewSubtitle } =
    useStores().messagingStore;
  ...
  return (
    <div className={classes.base}>
      ...
      <TextField
        ...
        onChange={(e) => {
          updateNewSubtitle(e.target.value);
        }}
        multiline
        rows={2}
        fullWidth
      ...
      {values.length > 0 && (
        <p className={classes.errorInfo}>{t("messaging.creation.errorInfo")}</p>
      )}
    </div>
  );
});
