...
const Messaging = observer(() => {
  ...
  const publicChannels =
    messagingStore.channels.length > 0
      ? messagingStore.channels.slice().filter((channel) => {
          return (
            !channel.isPrivate &&
  ...
  const coordinatorChannel =
    messagingStore.channels.length > 0
      ? [
          messagingStore.channels.find((channel) => {
      ...
      {!uiStore.onSpecificChannel ? (
        ...
        <>
          <OverTopBar
            ...
            title={
              messagingStore.selectedChannel.isCoordinatorChannel
                ? t("userTypes.coordinator")
                : messagingStore.selectedChannel.title
            }
          />
    ...
    channels = props.channels.map((channel) => {
      return (
        <ChannelPreview
          ...
          title={
            props.private ? `${t("userTypes.coordinator")}` : channel.title
          }
          ...
          unread={
            messagingStore.unreadInfo[channel.id]
          ...
          onClick={() => {
            ...
            messagingStore.selectedChannel.firstMessageId =
