...
const Channel = observer((props) => {
  ...
  useEffect(() => {
    if (!messagingStore.selectedChannel.id && uiStore.pathNumber > 0) {
      messagingStore.getChannelDetails(uiStore.pathNumber);
    }
  }, [uiStore.pathNumber]);
    ...
    <div className={classes.combined}>
      ...
      <div className={classes.inputContainer}>
        <MessageInput
          ...
          setValue={(value) => {
            messagingStore.newMessage = value;
          }}
          disableSend={
            messagingStore.newMessage === "" && messagingStore.file === ""
          }
...
const MessageList = observer((props) => {
  ...
  if (
    props.selectedChannel.messages &&
  ...
  return (
    <div
      ...
      style={{ marginTop: props.isCoordinator ? 0 : "60px" }}
    >
      <TopOfChannelDetails newestMessageRef={newestMessageRef} />
      {!messagingStore.initalMessagesLoaded ? (
        <LoadingChannel />
      ) : (
        <>
          {messages.length > 0 ? (
            ...
            <p className={classes.empty}>{t("messaging.empty")}</p>
          )}
        </>
      )}
...
const TopOfChannelDetails = observer(({ newestMessageRef }) => {
  ...
  return (
    <>
      {messagingStore.initalMessagesLoaded &&
        !messagingStore.selectedChannel.olderMessagesLoading &&
        !messagingStore.allMessagesLoaded && (
          <ClickableText
            icon={<HistoryIcon />}
            onClick={getOldMessages}
            text={t("messaging.loadMore")}
          />
        )}
      {messagingStore.selectedChannel.olderMessagesLoading && (
        <CircularProgress
