...
const Subtitle = ({ children }) => {
  const classes = useStyles();
  return (
    <p className={`${classes.noMargin} ${classes.subTitle}`}>{children}</p>
  );
};
...
const ChannelPreview = observer((props) => {
  ...
  const getSubtitle = () => {
    if (props.private)
      return <Subtitle>{t("messaging.privateExplained")}</Subtitle>;
    if (props.isSiteChannel)
      return <Subtitle>{t("messaging.clinicChat")}</Subtitle>;
  ...
  return (
    <Container {...props}>
      <div className="display">
        <span>{props.title ? props.title[0] : "C"}</span>
      </div>
      <BorderedPart hideBorder={props.coordinator}>
        <div>
          <Title>
            {props.title === "tb-expert-chat"
              ? t("messaging.expert")
              : props.title}
          </Title>
          {getSubtitle()}
        ...
        <div className="rightSideContainer">
          ...
          {props.coordinator && props.creatorIsArchived && (
            <Label
...
const BorderedPart = styled.div`
  border-bottom: ${(props) =>
    !props.hideBorder ? "solid 1px lightgray" : "unset"};
  display: flex;
  flex-grow: 1;
  padding: 0.5em;
