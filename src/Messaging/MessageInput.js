  ...
  const handleFileInput = (event) => {
    if (event.target.files.length > 0) {
      const fileType = event.target.files[0].name.slice(
        (Math.max(0, event.target.files[0].name.lastIndexOf(".")) || Infinity) +
          1
      );
          ...
          <div className={classes.container}>
            {messagingStore.file && (
              <div className={classes.imagePreview}>
                <img src={messagingStore.file}></img>
                <div className={classes.imageText}>
                  <p>{t("messaging.imageWillSend")}</p>
                  <ButtonBase
                    onClick={messagingStore.toggleImagePreview}
                    className={classes.viewLarger}
                  >
                    {t("coordinator.sideBar.expandPhoto")}
                  </ButtonBase>
                </div>
            ...
            <div className={classes.base}>
              ...
              <label htmlFor="icon-button-file">
                <IconButton
                  onClick={messagingStore.getUploadUrl}
                  color="primary"
