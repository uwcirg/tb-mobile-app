...
it("checks current photo day", () => {
    ...
    patientStore.photoSchedule[`${DateTime.local().plus({ days: 1 }).toISODate()}`] = true;
    expect(patientStore.isPhotoDay).toBe(true)
    expect(patientStore.checkPhotoDay(DateTime.local().minus({ days: 1 }))).toBe(false)
    expect(patientStore.checkPhotoDay(DateTime.local().plus({ days: 1 }))).toBe(true)
})
