...
const Selection = () => {
  ...
  return (
    <div className={classes.selectionContainer}>
      ...
      <LargeSelector
        onClick={loginStore.selectPatient}
        id="Patient"
        backgroundColor={Colors.blue}
      >
        <AccountBoxIcon />
        <span>{t("userTypes.patient")}</span>
      </LargeSelector>
      <LargeSelector
        onClick={loginStore.selectPractitioner}
        id="Practitioner"
        backgroundColor={Colors.blue}
      >
        <SupervisorAccountIcon />
        <span>{t("userTypes.provider")}</span>
      </LargeSelector>
    </div>
  );
};
...
const LoginRouter = observer(() => {
  ...
  return (
    <div className={`${classes.container} `}>
      ...
      {(loginStore.selectedUserType || loginStore.onForgotPassword) && (
        <IconButton
          className={classes.backContainer}
          onClick={loginStore.goHome}
        >
          <ChevronLeftOutlined className={classes.back} />
        </IconButton>
      )}
