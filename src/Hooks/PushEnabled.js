...
export default function usePushEnabled() {
  ...
  const checkAndSetState = () => {
    if (mounted.current) {
      //Ensure component is currently rendered to prevent memory leak error
  ...
  useEffect(() => {
    if (prevPref.current === "denied" && permissionState === "granted") {
      //If newly opted in, ensure that credentials are up to date
      patientStore.subscribeToNotifications();
    } else if (permissionState === "denied") {
      //Log change on server if newly denied, opt
