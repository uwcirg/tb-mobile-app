  ...
  return (
    <>
      {props.list.length > 0 ? (
        <div>
          {" "}
          {props.list.map((symptom) => {
            return (
              <p key={`report-symptom-list-${symptom}`}>
...
const FullReport = ({ row }) => {
  ...
  const timeTaken = row.takenAt
    ? DateTime.fromISO(row.takenAt).toLocaleString(DateTime.TIME_SIMPLE)
    : t("commonWords.yes");
  ...
  return (
    <Table size="small" aria-label="report-details">
      <TableBody className={classes.body}>
        <TableRow>
          <TableCell>{t("report.for")}</TableCell>
          <TableCell>
        ...
        <TableRow>
          ...
          <TableCell>
            <p>
              {DateTime.fromISO(row.createdAt).toLocaleString(DATETIME_FORMAT)}
            </p>
            {row.numberOfDaysAfterRequest > 0 && (
              <p>
                <span className={classes.highlight}>{`${
                  row.numberOfDaysAfterRequest
                } ${t("patient.report.dayLate", {
                  count: row.numberOfDaysAfterRequest,
                })}`}</span>
              </p>
            )}
          </TableCell>
        ...
        <TableRow>
          ...
          <TableCell>
            {row.medicationWasTaken ? (
              <p>
                {row.takenAt && (
                  <>{t("patient.report.confirmation.takenAt")}:</>
                )}{" "}
                {timeTaken}
              </p>
            ) : (
              <p>
                <span className={classes.warningHighlight}>
                  {t("coordinator.tasksSidebar.notTaken")}
                </span>
              </p>
            )}
            {!row.medicationWasTaken && (
              <>
                {row.whyMedicationNotTaken ? (
                  <p>
                    {t("coordinator.message")}: {row.whyMedicationNotTaken}
                  </p>
                ) : (
                  <p>{t("coordinator.sideBar.noReason")}</p>
                )}
              </>
            )}
          </TableCell>
        ...
        <TableRow>
          ...
          <TableCell>
            ...
            {row.doingOkayReason && (
              <p>
                {t("coordinator.message")}: {row.doingOkayReason}
              </p>
            )}
          </TableCell>
        </TableRow>
...
const PhotoRow = ({ row }) => {
  ...
  return (
    <>
      {!row.photoWasRequired ? (
        <p>{t("report.photoNotNeeded")}</p>
      ) : (
        <>
          {row.photoUrl ? (
            <>
              <div className={classes.photoArea}>
                <div>
                  ...
                  <Grid alignItems="flex-start" container>
                    <PhotoStatus
                      conclusive={
                        row.photoDetails && row.photoDetails.approvalStatus
                      }
                    />
                    <IconButton
                      className={classes.photoExpand}
              ...
              {row.photoDetails.createdAt && (
                <Grid alignItems="center" container>
                  ...
                  {row.photoDetails.backSubmission && (
                    <Tag
                      className={classes.photoLateTag}
                      backgroundColor={Colors.highlightYellow}
                    >
                      {t("patient.report.late")}
                    </Tag>
                  )}
                </Grid>
              )}
            ...
            <>
              <p>
                <span className={classes.warningHighlight}>
                  {t("report.missedPhoto")}
                </span>
              </p>
              <p>
                {" "}
                {row.whyPhotoWasSkipped ? (
                  <>
                    {t("coordinator.message")}: {row.whyPhotoWasSkipped}
                  </>
                ) : (
                  t("coordinator.sideBar.noReason")
                )}
              </p>
            </>
          )}
