...
const SymptomSummary = observer(() => {
  ...
  const symptomSummary =
    useStores().patientProfileStore.selectedPatient.symptomSummary;
  ...
  const list =
    Object.keys(symptomSummary).length > 0
      ? Object.keys(symptomSummary[selection])
      : [];
  const total =
    list.length > 0
      ? Object.values(symptomSummary[selection]).reduce((acc, cur) => {
          return acc + cur;
        })
      : 0;
      ...
      <div className={classes.top}>
        ...
        >
          {options.map((each, index) => {
            return (
              <MenuItem key={`symotom-summary-${index}`} value={each}>
                <span className={classes.menuItem}>
                  {t(
                    `coordinator.patientProfile.symptomSummary.timeOptions.${each}`
                  )}
                </span>
              </MenuItem>
            );
        ...
        <div className={classes.totalContainer}>
          <div className={classes.barContainer}>
            ...
            {list
              .sort((a, b) => {
                return (
                  symptomSummary[selection][b] - symptomSummary[selection][a]
                );
              })
              .map((each) => {
                ...
                return (
                  <React.Fragment key={`${each}`}>
                    {symptomSummary[selection][each] > 0 ? (
                      <>
                        <div className={classes.barLabel}>
                          <p>{t(`symptoms.${each}.title`)}</p> <p>{value}</p>
                        </div>
                        <BorderLinearProgress
                          severe={SevereSymptoms.includes(each)}
                          variant="determinate"
