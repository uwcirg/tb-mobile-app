...
const AddNote = observer(({ handleClose }) => {
  ...
  return (
    <PopOver
      close={handleClose}
      title={`${t("coordinator.patientProfile.addNote.header")} ${
        patientProfileStore.selectedPatient.details.givenName
      }`}
    >
      <div className={classes.container}>
        ...
        />
        <MuiButton disabled={!title || !note} onClick={submitNote}>
          {t("coordinator.patientProfile.addNote.button")}
        </MuiButton>
      </div>
    </PopOver>
  );
