  ...
  return (
    <>
      {patientProfileStore.isArchived && (
        <Grid container direction="column" className={classes.archived}>
          <Warning />
          <TreatmentOutcomes />
        </Grid>
      )}
...
const TreatmentOutcomes = observer(() => {
  const classes = useStyles();
  const { patientProfileStore } = useStores();
  const { treatmentOutcome, appEndDate } =
    patientProfileStore.treatmentOutcomes;
  const { t } = useTranslation("translation");
  ...
  const handleClick = () => {
    ...
    history.push(
      `/patients/${patientProfileStore.selectedPatient.details.id}/archive`
    );
  };
  ...
  return (
    <div className={classes.details}>
      <OutcomeSection
        title={t("archive.treatmentOutcome")}
        body={
          treatmentOutcome
            ? t(`archive.outcomeTypes.${treatmentOutcome}`)
            : t("archive.notSelected")
        }
      />
