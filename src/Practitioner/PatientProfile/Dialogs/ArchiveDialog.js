...
const ArchiveDialog = observer(({ handleClose }) => {
  ...
  const allowSubmission =
    patientProfileStore.treatmentOutcome.appEndDate &&
    patientProfileStore.treatmentOutcome.treatmentOutcome;
  ...
  return (
    <PopOver
      title={t("coordinator.patientProfile.options.archive")}
      ignoreClickAway
      close={handleClose}
    >
      <Typography variant="body1">{t("archive.explanation")}</Typography>s
      <form className={classes.form}>
        <DatePicker
          ...
          onChange={(datetime) => {
            patientProfileStore.treatmentOutcome.appEndDate = datetime.toISO();
          }}
        />
        <SelectOutcome />
      </form>
      <div className={classes.bottomButton}>
        {!allowSubmission && (
          <Typography variant="body1">{t("commonWords.fillAll")}</Typography>
        )}
...
const SelectOutcome = observer(() => {
  const { treatmentOutcome, setTreatmentOutcome } =
    useStores().patientProfileStore;
  const classes = useStyles();
  return (
    <TreatmentOutcomeSelection
