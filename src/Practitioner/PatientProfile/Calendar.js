...
*/
    ...
    return(
        <div className={`${classes.day} ${compositeClass}`}>
            {!props.disabled && <div className={classes.badgeContainer}>{tookMeds ? <Check style={{color: Colors.approvedGreen}}/> : <Clear style={{color: Colors.red}}  /> }</div>}
            {selectedDay ? <div className={classes.selectedDay}><p>{props.date}</p> </div> : <div className={classes.dateNum}>{props.date}</div>}
