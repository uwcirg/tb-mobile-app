...
const useStyles = makeStyles({
  ...
  element: {
    ...
    backgroundColor: (props) =>
      props.completed ? Colors.accentBlue : Colors.gray,
    position: 'relative',
  },
  endsLayout: {
    width: '100%',
...
const ProgressVis = observer(() => {
  ...
  const end = getDate(
    patientProfileStore.selectedPatient.details.treatmentEndDate
  );
...
const EndsLayout = ({ start, end, week }) => {
  ...
  return (
    <Grid
      ...
      container
      wrap="nowrap"
      className={classes.endsLayout}
    >
      <Item
        top={`${t('coordinator.patientTableLabels.treatmentStart')}:`}
        bottom={start}
      />
      <Item
        className={classes.middleLabel}
        top={`${t('commonWords.total')}:`}
        bottom={`Week ${week}`}
      />
      <Item top={`${t('patient.profile.endDate')}:`} bottom={end} />
    </Grid>
  );
};
...
const Visualization = ({ treatmentWeek }) => {
  ...
  while (i < 26) {
    //Iterate over the 26 weeks of treatment
    if (i === treatmentWeek) {
      list.push(
        <Element key={`week-${i}`} current={true} completed={true} value={i} />
      );
      markerSet = true;
