...
const Buttons = ({ state, setState, submitReview }) => {
  ...
  const handleChange = (e) => {
    let tempState = { ...state };
    tempState[e.target.name] =
      e.target.name === "redoFlag" ? e.target.checked : e.target.value;
    setState(tempState);
  };
  ...
  return (
    <Grid className={classes.form} container direction="column">
      ...
      <Grid style={{ paddingTop: "2em" }} container>
        ...
        <Box width=".5em" />
        <FlatButton onClick={handleNext} disabled={!enableSubmit}>
          {showResubmissionOption ? "Continue" : "Submit"}
        </FlatButton>
      </Grid>
    </Grid>
  );
...
const ReviewPhotoPopOver = ({
  ...
  const reviewPhoto = async () => {
    return PractitionerAPI.reviewPhoto(photo.photoId, {
      ...state,
      approved: getApprovedValue(state.approved),
    });
  };
