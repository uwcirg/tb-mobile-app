...
const ResetCode = ({ patient }) => {
  ...
  const launchReset = () => {
    practitionerStore.resetActivationCode(patient.id);
  };
  const { t } = useTranslation("translation");
  const classes = useStyles();

  return (
    <FlatButton className={classes.button} onClick={launchReset}>
      <RefreshIcon />
...
const fields = [
  ...
  {
    displayName: (
      <Translate string="coordinator.addPatientFlow.activationCode" />
    ),
    disableSorting: true,
    align: "right",
