...
const AddPatientForm = observer((props) => {
  ...
  const submitDisabled =
    !practitionerStore.newPatient.params.givenName ||
    !practitionerStore.newPatient.params.givenName ||
    practitionerStore.newPatient.params.phoneNumber.length < 5;
  ...
  return (
    <div className={classes.newPatientForm}>
      <form className={classes.inputBody} noValidate autoComplete="off">
        ...
        <br />
        <Typography className={classes.formLabels}>
          {t("coordinator.patientProfile.phoneNumber")}
        </Typography>
        <PatientInput id="phoneNumber" />
        <Typography className={classes.formLabels}>
          {t("patient.userFields.treatmentStart")}
        </Typography>
      ...
      >
        {submitDisabled && (
          <Typography style={{ maxWidth: "50%", marginRight: "auto" }}>
            {t("commonWords.fillAll")} *
          </Typography>
        )}
        <FlatButton
