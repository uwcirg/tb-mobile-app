...
const Adherence = observer(() => {
  ...
  labels.push(
    <span key={`background-label-0`} className="yLabel">
      0%
    </span>
  );
  labels.push(
    <span key={`background-label-1`} className="yLabel">
      90%
    </span>
  );
  labels.push(
    <span key={`background-label-2`} className="yLabel">
      100%
    </span>
  );
  ...
  for (let step = 0; step < 6; step++) {
    months.push(
      <div key={`background-month-${step}`} className="month">
        {step + 1}
      </div>
    );
      ...
      <div className={classes.xLabelAndGraph}>
        <div className={classes.container}>
          ...
          <div className={classes.top}>
            {top.map((each, i) => {
              return <DataPoint key={`datapoint-top-${i}`} {...each} top />;
            })}
          </div>
          <div className={classes.bottom}>
            {bottom.map((each, i) => {
              return <DataPoint key={`datapoint-bottom-${i}`} {...each} />;
            })}
          </div>
        </div>
...
const DataPoint = (props) => {
  ...
  if (props.top) {
    bottomCalc =
      props.adherence > 0
        ? `calc(${((props.adherence - divider) / (1 - divider)) * 100}%)`
        : "0px";
  } else {
    bottomCalc =
      props.adherence > 0
        ? `calc(${(props.adherence / divider) * 100}%)`
        : "0px";
  }
    ...
    <Tooltip
      title={
        <>
          ...
          <p>
            {t("coordinator.adherence")}: {Math.round(props.adherence * 100)}%
          </p>
          <p>
            {t("coordinator.daysInTreatment")}: {props.daysInTreatment}
          </p>
        </>
      }
    >
      <div
        style={{
          bottom: bottomCalc,
          left: `${props.percentageComplete * 100}%`,
        }}
  ...
  return (
    <div>
      <div className={classes.visContainer}>
        <div className={classes.backgroundContainer}>{column}</div>
