...
const useStyles = makeStyles({
  ...
  circle: (props) => ({
    height: "15px",
    width: "15px",
    backgroundColor: props.medicationWasTaken
      ? Colors.approvedGreen
      : Colors.red,
    borderRadius: "50%",
    marginRight: "5px",
  }),
...
const RecentReports = observer((props) => {
  ...
  return (
    <div className={classes.container}>
      ...
      {practitionerStore.recentReports.length > 0 && (
        <Reports reports={practitionerStore.recentReports} />
      )}
    </div>
  );
});
      ...
      <>
        ...
        <div className={classes.reportRow}>
          ...
          <div className={classes.report}>
            <Patient
              {...practitionerStore.getPatient(report.userId)}
              medicationWasTaken={report.medicationWasTaken}
            />
            <p>
              {report.medicationWasTaken
                ? t("coordinator.recentReports.tookMeds")
                : t("coordinator.recentReports.didntTakeMeds")}
            </p>
            <Symptoms list={report.symptoms} />
            {report.photoUrl ? (
              <img className={classes.image} src={report.photoUrl} />
            ) : (
              <p>{t("coordinator.recentReports.noPhoto")}</p>
            )}
          </div>
        </div>
      </>
