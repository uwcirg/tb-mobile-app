...
const PhotoSidebar = observer(() => {
  ...
  const item =
    practitionerStore.filteredPatients.photo[
  ...
  const medicationValue =
    state.approved === null ? null : state.approved ? "true" : "false";
      ...
      <Box padding="1em">
        <Grid alignItems="center" container className={classes.titleContainer}>
          <Typography className={classes.title}>
            <strong>{t("coordinator.sideBar.photoSub")}:</strong>
          </Typography>
          <Box flexGrow="1" />
          {item.backSubmission && (
            <Label
              ...
              text={
                item.isRedo ? t("redoPhoto.flag") : t("patient.report.late")
              }
            />
          )}
        </Grid>
        ...
        {item.isRedo && (
          <Typography>
            <strong>{t("redoPhoto.newSubmission")}:</strong>
          </Typography>
        )}
        ...
        <FormControl component="fieldset">
          <FormLabel className={classes.resultLabel} component="legend">
            {t("redoPhoto.selectResult")}
          </FormLabel>
          <RadioGroup
            name="approved"
              ...
              <FormControlLabel
                ...
                label={
                  <Grid container alignItems="center">
                    <PlusIcon />
                    <Typography>{t("redoPhoto.detected")}</Typography>
                  </Grid>
                }
              />
              <FormControlLabel
                ...
                label={
                  <Grid container alignItems="center">
                    <MinusIcon />
                    <Typography>{t("redoPhoto.notDetected")}</Typography>
                  </Grid>
                }
              />
            </Grid>
        ...
        <Fade in={state.redoFlag}>
          <div>
            ...
            <TextField
              ...
              fullWidth
              onChange={(e) => {
                setState({ ...state, redoReason: e.target.value });
              }}
              value={state.redoReason}
              multiline
              rows={2}
            />
          </div>
...
const SubmitButon = ({ enabled, reviewState, photoReport, resetState }) => {
  ...
  const handleSubmit = () => {
    practitionerStore
      .processPhoto(photoReport.photoId, reviewState)
      .then((res) => {
        if (!res.photoId) {
          uiStore.setAlert(
            "Error updating photo report. Please try again",
            "error"
          );
          return;
        }
...
const LateSubmissionInfo = ({ photoReport }) => {
  ...
  const requestDate = DateTime.fromISO(photoReport.date).toLocaleString(
    DATETIME_MED_NO_YEAR
  );
  const submittedDatetime = DateTime.fromISO(
    photoReport.createdAt
  ).toLocaleString(DATETIME_MED_NO_YEAR);

  return (
    <div className={classes.lateArea}>
      <Typography>
        {t("dashboard.requested")}: {requestDate}
      </Typography>
      <Typography>
        {t("dashboard.submitted")}: {submittedDatetime}
      </Typography>
    </div>
  );
};
