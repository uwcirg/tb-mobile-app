    ...
    list = practitionerStore.filteredPatients.photo.map((report) => {
      return (
        <div>
          ...
          <div>{report.photo_id}</div>
          <IconButton
            onClick={() => {
              practitionerStore.processPhoto(report.photo_id, true);
            }}
          >
            <CheckIcon />{" "}
          </IconButton>
          <IconButton
            onClick={() => {
              practitionerStore.processPhoto(report.photo_id, false);
            }}
          >
            {" "}
            <XIcon />
...
const ProcessedPhotoList = observer((props) => {
  ...
  let list = "";
  if (practitionerStore.processedPhotoReports.length > 0)
    list = practitionerStore.processedPhotoReports.map((report) => {
      return (
        <div>
          <img className={classes.stripPhoto} src={report.url} />
          <div>{report.photo_id}</div>
  ...
  return (
    <>
      ...
      <button onClick={practitionerStore.getProcessedPhotoReports}>
        Update
      </button>
      <div>{list}</div>
    </>
