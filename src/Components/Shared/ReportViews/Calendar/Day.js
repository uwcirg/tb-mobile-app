...
const getSurroundingDates = (datetime) => {
  return [
    datetime.startOf("day").minus({ days: 1 }),
...
const Day = ({ dateObj, reports, treatmentStart, disabled, date }) => {
  ...
  const getReportColorForDate = (date) => {
    const isWithinTreatmentBounds =
      date.diff(
        DateTime.fromISO(DateTime.fromISO(treatmentStart).toISODate()),
        "days"
      ).days >= 0 && datetime.diffNow("days").days < 0;
  ...
  const getRelevantReportColors = () => {
    ...
    return [
      getReportColorForDate(prevDate),
      getReportColorForDate(currentDate),
      getReportColorForDate(nextDate),
    ];
  };

  const [prevColor, currentColor, nextColor] =
    getRelevantReportColors(datetime);

  const relevantDay = new CalendarDayStyleHelper(
    reports[datetime.toISODate()],
  ...
  return (
    <div
      ...
      className={`${classes.day} ${!disabled && classes.nonDisabledDay} ${
        relevantDay.rightRounding && classes.end
      } ${relevantDay.leftRounding && classes.start}`}
    >
      <p className={`${isToday && classes.today}`}>{date}</p>
      <div className={classes.bottomDots}>
        {relevantDay.modifiers.map((each) => (
          <div
            key={`${datetime.toISODate}-mod-${each}`}
