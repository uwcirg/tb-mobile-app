  ...
  return (
    <PopUp handleClickAway={handleExit}>
      ...
      <h1 className={classes.title}>
        {t("patient.reportConfirmation.goodJob")} {patientStore.givenName}!
      </h1>
      <p>{t("patient.reportConfirmation.photoNeeded")}</p>
      <NewButton
        onClick={handleClick}
        icon={<Camera />}
        text={t("patient.home.actions.uploadPhoto")}
      />
      <Button className={classes.basicButton} onClick={handleExit}>
        {t("patient.reportConfirmation.doLater")}
      </Button>
    </PopUp>
  );
});
