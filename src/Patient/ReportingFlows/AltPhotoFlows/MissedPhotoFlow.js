...
const MissedPhotoFlow = observer(() => {
  ...
  const handleSubmit = () => {
    setLoading(true);
    new PatientInformationAPI()
      .submitBackPhotoReport(
        photo,
  ...
  const requestDateFormatted = DateTime.fromISO(
    patientStore.lastPhotoRequestStatus.dateOfRequest
  ...
  return (
    <>
      <OverTopBar
        ...
        title={t("missedPhotoDetails.topBarText")}
      />
      <div className={classes.container}>
        {response ? (
          <PostSubmissionView response={response} />
        ) : (
          <PreSubmissionView
