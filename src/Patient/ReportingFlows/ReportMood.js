...
const ReportMood = observer((props) => {
  ...
  const continueIsDisabled =
    !patientStore.report.doingOkaySelected ||
    (!patientStore.report.doingOkay &&
      patientStore.report.supportReason === "");

  const goodSelected =
    patientStore.report.doingOkaySelected && patientStore.report.doingOkay;
  const badSelected =
    patientStore.report.doingOkaySelected && !patientStore.report.doingOkay;
        ...
        {badSelected && (
          <div className={classes.bottom}>
            <TextField
              ...
              rows={3}
              multiline
              label={t("patient.report.whySupport")}
              onChange={(e) =>
                (patientStore.report.supportReason = e.target.value)
              }
              value={patientStore.report.supportReason}
            />
            {continueIsDisabled && (
              <WarningBox className={classes.warning}>
