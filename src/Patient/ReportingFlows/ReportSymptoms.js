  ...
  return (
    <>
      <Box padding="8px 16px">
        <SymptomsList
          symptomLocale={isIndonesiaPilot() ? "indonesia" : "argentina"}
        />
      </Box>
      <SimpleButton alignRight className={classes.button} onClick={handleNext}>
        {t("commonWords.report")}{" "}
        {patientStore.report.selectedSymptoms.length === 0
          ? t("commonWords.no")
          : patientStore.report.selectedSymptoms.length}{" "}
        {patientStore.report.selectedSymptoms.length === 1
          ? symptoms.substring(0, symptoms.length - 1)
          : symptoms}
      </SimpleButton>
