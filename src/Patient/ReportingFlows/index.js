...
const MedicationFlow = observer(() => {
  ...
  const topDateString = DateTime.fromISO(
    patientStore.report.date
  ).toLocaleString(format);

  return (
    <div className={classes.container}>
      {!patientUIStore.skippedToPhotoFlow && (
        <NumberedTitle
          number={tabNumber}
          title={patientStore.report.headerText}
        />
      )}
      <OverTopBar
        title={
          patientUIStore.onHistoricalReport
            ? `${t("report.for")} ${topDateString}`
            : t("patient.report.topTitle")
        }
