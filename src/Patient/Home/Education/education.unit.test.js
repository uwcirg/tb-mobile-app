...
const updateOpenState = () => {
  ...
  stores.patientStore.educationStore.setLocalToOldDateForTesting(
    DateTime.local().minus({ days: 5 }).toISODate()
  );
  stores.patientStore.educationStore.updateCurrentDate();
  stores.patientStore.educationStore.dateOfLastUpdateRead = DateTime.local()
    .minus({ days: 5 })
    .toISODate();
};
...
test("update appears when its a new day", async () => {
  stores.patientStore.educationStore.updateCurrentDate();
  stores.patientStore.educationStore.setDateOfLastUpdateRead(
