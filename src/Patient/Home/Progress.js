...
const ProgressGraph = observer(() => {
  ...
  const dayValue =
    (patientStore.patientInformation.daysInTreatment / 180) * 100;
          ...
          <div className={classes.graph}>
            ...
            >
              <p className={classes.progressText}>
                <span>{patientStore.patientInformation.daysInTreatment}</span>{" "}
                {t("time.days")}
              </p>
            </CircularProgressbar>
          </div>
          <StatBox
            title={patientStore.getCurrentStreak}
            text={t("patient.home.progress.currentStreak")}
          />
        </div>
      </div>
