...
const RequiresAction = observer(() => {
  ...
  const [surveyHidden, setSurveyHidden] = useLocalValue(
    "appSurveyHidden",
    true
  );
  ...
  const shouldShowAppSurvey =
    patientStore.patientInformation.weeksInTreatment >= 20 &&
    !surveyHidden &&
    !isIndonesiaPilot();

  const shouldRender =
    !uiStore.offline &&
    (shouldShowMissedReports ||
      shouldShowContactTracing ||
      shouldShowMissedPhoto ||
      shouldShowAppSurvey ||
      shouldShowRedoPhoto);
      ...
      {shouldRender && (
        <HomePageSectionContainer
          upperText={
            <>
              <WarningIcon />
              {t("patient.home.cardTitles.actionNeeded")}
            </>
          }
        >
