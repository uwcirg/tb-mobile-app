...
const useStyles = makeStyles({
  ...
  reportOption: {
    ...
    backgroundColor: (props) =>
      props.yes ? Colors.calendarGreen : Colors.highlightYellow,
  },
  header: {
...
const MissedReports = observer(() => {
  ...
  return (
    <MissedActionCard id="intro-missed">
      <ButtonLayout
        text={`${patientStore.missingReports.length} ${t(
          "patient.home.missedDays.missing",
          { count: patientStore.missingReports.length }
        )}`}
      ...
      <Grow in={showDetails} className={classes.grow}>
        <div className={classes.content}>
          <Grid
            alignItems="center"
            className={`${classes.oneStep} ${classes.header}`}
            container
          >
          ...
          {patientStore.missingReports.map((date, index) => {
            return (
              <OneStepBackReport key={`back-report-${date}`} date={date} />
            );
          })}
