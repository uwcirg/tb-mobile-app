...
const CachedReports = observer(() => {
  ...
  const shouldRender =
    (!uiStore.offline && dailyReportStore.finished) ||
    dailyReportStore.syncing ||
        ...
        <div className={classes.container}>
          {!uiStore.offline && dailyReportStore.finished ? (
            <div className={classes.finished}>
              {t("patient.report.offline.allDone")}{" "}
              <FinishedIcon
                style={{ color: Colors.approvedGreen, marginLeft: "5px" }}
              />
            </div>
          ) : (
            <>
              ...
              {!dailyReportStore.syncing &&
                dailyReportStore.numberOfflineReports > 0 && <OfflineMessage />}
              {dailyReportStore.syncing && (
                <p className={classes.message}>
                  {t("patient.report.offline.uploading")}...{" "}
                </p>
              )}
            </>
          )}
...
const OfflineMessage = observer(() => {
  ...
  return (
    ...
    >
      ...
      <p className={classes.message}>
        {t("patient.report.offline.youHave")}{" "}
        {dailyReportStore.numberOfflineReports}{" "}
        {t("patient.report.offline.report", {
          count: dailyReportStore.numberOfflineReports,
        })}
        . {t("patient.report.offline.explanation")}
      </p>
    </Grid>
  );
});
