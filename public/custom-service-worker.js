...
try {
  ...
  console.log(
    "Error Caught - Offline Precaching will not work in CRA development mode. Edit to test  "
  );
}
...
self.addEventListener("notificationclick", function (event) {
  ...
  const promiseChain = clients.matchAll().then((windowClients) => {
    ...
    if (matchingClient) {
      ...
      event.notification.close();
      return clients.openWindow(redirectURL).then(function (client) {
        client.focus();
      });
    }
  });
...
function logNotificationDelivery(notificationId) {
  callAnalyticsAPI(notificationId, {
    deliveredSuccessfully: true,
    deliveredAt: getCurrentISODateTime(),
  });
}
...
function logNotificationClick(notificationId) {
  callAnalyticsAPI(notificationId, {
    clicked: true,
    clickedAt: getCurrentISODateTime(),
  });
}
